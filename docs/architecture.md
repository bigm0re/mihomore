...
`mihomore` 现在是 Rust workspace，按稳定边界拆成多个 crate。依赖方向表达架构约束：`air-ui` 收集输入并展示状态，调用 `air-app` 的命令入口；`air-app` 负责任务调度、状态快照和事件广播；`air-mihomo` 统一承载 mihomo 进程、external-controller API、订阅/代理/规则领域模型与相关业务逻辑；`air-storage`、`air-platform` 是外部资源边界。

> 注：crate 前缀仍为 `air-`（历史命名），对外产品名统一为 `mihomore`。

## Workspace 结构

```text
crates/
  air-desktop/      # 最终 mihomore binary、allocator、构建期 mihomo 下载和 Windows 可执行文件图标资源
  ...
  air-platform/     # tray/autostart/service/UAC/window/single-instance/system-proxy
...
- `air-storage -> air-config + air-mihomo + air-settings + air-paths + air-error`
- `air-config -> air-error + air-telemetry`
- `air-platform -> air-error + air-paths + air-telemetry`
- `air-settings -> air-error`
- `air-telemetry -> air-paths + air-error`
- `air-paths -> air-error`
...
- `air-paths` 只做目录解析，不依赖 `air-storage`/`air-platform`；它是三者共同的下层，避免形成依赖环。
